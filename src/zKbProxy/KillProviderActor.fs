...
type KillProviderActor(log: PostMessage, stats: PostMessage, config: Configuration, name: string)=
    ...
    let dbCollection = MongoDb.killsCollection config.MongoServer config.DbName config.KillsDbCollection config.MongoUserName config.MongoPassword
    ...
    let pipe = MessageInbox.Start(fun inbox ->
        ...
        loop({ KillCache.Empty config.BufferSize with LastPull = DateTime.UtcNow })
    )
